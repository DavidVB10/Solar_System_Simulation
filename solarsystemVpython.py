...
text=[textSun,textMer,textVen,textTie,textMar,textJup,textSat,textUra,textNep]
...
for j in range(len(trayectorias)):
		...
		y=cuerpo[1]
		z=cuerpo[2]
		bola=sphere(pos=vector(x[0]/AU*10,y[0]/AU*10,z[0]/AU*10),radius=tam[j]/5,texture={'file':text[j],'place':'all'})
		planetas.append(bola)
		
		
for i in range(len(trayectorias[0][0])):
	for j in range(len(trayectorias)):
		cuerpo = trayectorias[j]
		x=cuerpo[0]
		y=cuerpo[1]
...
for j in range(len(trayectorias)):
		...
		y=cuerpo[1]
		z=cuerpo[2]
		bola=sphere(pos=vector(x[0]/AU*10,y[0]/AU*10,z[0]/AU*10),radius=tam[j],texture={'file':text[j],'place':'all'})
		planetas.append(bola)
...
for i in range(len(trayectorias[0][0])):
	for j in range(len(trayectorias)):
		...
		z=cuerpo[2]
		bola=planetas[j]
		bola.pos = (x[i]/AU*10,y[i]/AU*10,z[i]/AU*10)
		rate(10)
		bola.visible=False
...
for i in range(len(trayectorias[0][0])):
	for j in range(len(trayectorias)):
		cuerpo = trayectorias[j]
		x=cuerpo[0,:int(seg[j]/nin)]
		y=cuerpo[1,:int(seg[j]/nin)]
		z=cuerpo[2,:int(seg[j]/nin)]
		bola=sphere(pos=vector(x[i]/AU*10,y[i]/AU*10,z[i]/AU*10),radius=tam[j],texture={'file':text[j],'place':'all'})
		rate(100)
